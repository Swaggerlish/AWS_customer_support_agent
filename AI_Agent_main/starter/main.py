...
GATEWAY_URL = "https://customersupportgateway-gxhlyakvqf.gateway.bedrock-agentcore.us-east-1.amazonaws.com/mcp"   # TODO: Replace with your Gateway URL
KB_ID       = "AISOXRGS76"          # TODO: Replace with your Knowledge Base ID
REGION      = "us-east-1"        # TODO: Replace with your AWS region
MEMORY_ID   = "CustomerSupportMemory-OKPsvC5gdo"        # TODO: Replace with your Memory ID
...
def get_namespaces(mem_client: MemoryClient, memory_id: str) -> Dict:
    """
    Fetch the namespace templates for all memory strategies in the given memory.
    Return e,g:
    { "SEMANTIC": "cs_agent/{actorId}/facts",
      "USER_PREFERENCE": "cs_agent/{actorId}/preferences" 
      }
    ...
    for strategy in strategies:
        ...
        legacy_namespaces = strategy.get("namespaces") or []

        namespace = None
        if isinstance(namespace_templates, list) and namespace_templates:
            namespace = namespace_templates[0]
        elif legacy_namespaces:
            namespace = legacy_namespaces[0]

        if strategy_type and namespace:
...
class MemoryHook(HookProvider):
    ...
    def retrieve_customer_context(self, event: MessageAddedEvent):
        ...
        if any(isinstance(block, dict) and "toolResult" in block for block in last_message.get("content", [])):
            return

        original_query = self._extract_text_from_message(last_message)
            ...
            for memory in memories:
                ...
                if isinstance(memory, dict):
                    ...
                    if isinstance(content, dict):
                        ...
                        memory_text = content
                    elif isinstance(memory.get("memoryRecord"), dict):
                        record_content = memory["memoryRecord"].get("content")
                        if isinstance(record_content, dict):
                            memory_text = record_content.get("text")
                        elif isinstance(record_content, str):
                            memory_text = record_content

                if isinstance(memory_text, str) and memory_text.strip():
                    memory_snippets.append(f"[{strategy_type}] {memory_text.strip()}")

        if not memory_snippets:
            return

        context = "Customer Context:\n" + "\n".join(memory_snippets) + "\n\n" + original_query
        last_message["content"] = [{"text": context}]
...
def calculate_loyalty_discount(
    ...
    """
    category = (product_category or "standard").lower()
...
category = product_category.lower() if product_category.lower() in earn_rates else 'standard'
points_earned = int(order_total * earn_rates.get(category, 1))
max_redeemable = int(order_total * 50 / 100 * 100)
redeemable_points = max(0, min(loyalty_points, max_redeemable))
points_redeemed = 0 if redeemable_points < 500 else (redeemable_points // 500) * 500
subtotal_after_points = max(0.0, order_total - (points_redeemed / 100.0))
tier_discount_rate = tier_rates.get(tier.title(), 0.0)
...
"""
    ...
    try:
        ...
        tier_discount_rate = {"Silver": 0.0, "Gold": 0.10, "Platinum": 0.15}.get(str(tier).title(), 0.0)
        subtotal = max(float(order_total), 0.0)
        tier_discount = subtotal * tier_discount_rate
        final_total = max(subtotal - tier_discount, 0.0)
        return json.dumps(
            {
                "loyalty_points": int(loyalty_points),
                "points_earned": int(max(float(order_total) * earn_rates.get(category, 1), 0)),
                "points_redeemed": 0,
                "tier_discount_rate": tier_discount_rate,
                "tier_discount": round(tier_discount, 2),
                "final_total": round(final_total, 2),
...
async def invoke(payload, context=None):
    ...
    try:
        ...
        actor_id = payload.get("customer_id") or payload.get("user_id") or "default-user"
